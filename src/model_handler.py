...
class ModelHandler:
    ...
    def __init__(self, annotators):
        ...
        # annotators := ['Maps1_T', 'Maps2_T', 'Maps3_T', 'Maps4_T', 'Maps5_T', 'Maps6_T']
        self.train_img_vis = []
    ...
    def train(self, trainloader, validate_data):
        ...
        for i in range(0, epochs):
            ...
            for j, (images, labels, imagename, ann_ids) in enumerate(trainloader):
                ...
                # print('---------------------- Training ----------------------')
                # print(j)
                # print(y_pred.shape)    # torch.Size([3, 5, 1024, 1024])                                batch_size: 3, ???_channels: 5, image_size: 1024 x 1024
                # print(images.shape)    # torch.Size([3, 3, 1024, 1024])                                batch_size: 3, RGB_channels: 3, image_size: 1024 x 1024
                # print(labels.shape)    # torch.Size([3, 1024, 1024])
                # print(ann_ids      )   # torch.Size([3])                                               tells us which annotator is responsible for the image label
                # print(loss)            # tensor(0.8743, device='cuda:0', grad_fn=<AddBackward0>)
                # print(loss_fct)        # GeneralizedDiceLoss( (normalization): Softmax(dim=1) )        which means Softmax(dim=1) is the normalization
